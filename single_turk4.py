...
import chess
from chess import svg, Move, Board
...
class Turk:
    def __init__(self, v):
        ...
        self.vote_time_grain = .001 #There are 1/ grain is the number of ways the timer can be displayed
        #Bools
        ...
        self.css = {"input": "input[class='ready']", "moves_area": "div[class='rmoves']", "status" : "p[class='status']",
                "login": "a[href='/login?referrer=/']", "user": "input[name='username']", "pass": "input[name='password']",
                "ai": "a[href='/setup/ai']","button_white": "button[value='white']",
                "orientation_b": "div[class='cg-wrap orientation-black manipulable']", "orientation_w": "div[class='cg-wrap orientation-white manipulable']"}
        #Stream
        self.pic_path = "pics"
        ...
        graph = sg.Graph(self.graph_size, self.graph_bl, self.graph_tr, background_color = "#FFFFFF", pad = (0,0), key = "graph")
        self.votes_window = sg.Window("Votes", [[graph]], location = self.votes_window_location,  element_padding = (0,0), background_color = "#FFFFFF",
        ...
        self.timer_window = sg.Window("Timer", [[timer_graph]], location = self.timer_window_location, element_padding = (0,0), margins = (0,0),
                                        background_color = "#EDEBE9", no_titlebar=True, keep_on_top=True, grab_anywhere=True)
        ...
        self.chat_window = sg.Window("Chat", chat_layout, location = self.chat_window_location, element_padding = (0,0), background_color = "#FFFFFF", no_titlebar=True, keep_on_top=True, grab_anywhere=True)
    ...
    def skip_next_msg(self):
        ...
        #Handle PING
        if in_str[0:4] == "PING":
            self.sock.send(f"PONG".encode("utf-8"))
    ...
    #Selenium/Board
    ###############
    def handle_element_by_css_selector(self, css_selector : str, timeout, click = False, keys = None, sub_selector = None, get = False):
    ...
    def handle_moves_list(self, timeout = 10, returning = 'list'):
        ...
        while True:
            try:
                elements = self.handle_element_by_css_selector(self.css["moves_area"], timeout, get = True).find_elements_by_css_selector("m2")
                if returning == 'len':
                    return len(elements)
                for e in elements:
                    moves.append(e.text)
    ...
    def edit_name(self):
        while True:
            try:
                ...
                ("Couldn't find all 4 names. Trying again in 2 seconds.")
                time.sleep(2)
    ...
    def update_votes_window(self):
        #Graph vars
        max = int(self.graph_tr[0] *.85)
        separator = 2
        width = int(max - 9* separator)/10       
        #Color vars (A RGB value has to be >16 since handling hex(<16) is not important enough to take up lines)
        rgb_start = (230, 240, 249)
        rgb_end = (40, 97, 249)
        rgb_lin = [np.linspace(rgb_start[0], rgb_end[0], max+1).astype(int),
                        np.linspace(rgb_start[1], rgb_end[1], max+1).astype(int),
                        np.linspace(rgb_start[2], rgb_end[2], max+1).astype(int)]
        ...
        for i in range(min(len(ratio_series), 10)):
            ...
            graph.DrawText(text=ratio_series.index[i], location=(max*0.15, y + (width * .5)),
                        font = "Roboto 18")
            graph.DrawText(text=f"{round(ratio * 100, 2)}%", location=(max * 1.1, y + (width * .5)),
                        font = "Roboto 10")
        self.votes_window.Read(timeout=10)
        return True
    
    def update_timer_window(self, delta: int):
        #Graph vars
        ...
        return True
        # self.pic_path = "/base"
        #         self.pictures = ["chars_black", "nums_black", "chars_white", "nums_white"]
        #         self.pics_targets = ["chars_obs", "nums_obs"]
    ...
    def test_colors(self):
        ...
        for i in range(int(max/10)):
            ...
            graph.DrawRectangle(top_left=(0, y),
                                bottom_right = (bar_val, y + width),
                                fill_color= hex_color, line_color = hex_color)
            graph.DrawText(text=f"{bar_val/100}%", location=(max * 0.5, y + (width * .5)),
                        font = "Roboto 20")
            self.votes_window.Read(timeout=10)
        return True
    ...
    def run(self):
        ...
        game_input = input("Start a game and then press any key to continue")
        vote_time = timedelta(seconds = self.vote_time_int)
        self.update_votes_window()       
        #*Setup stream*
        time.sleep(1)     
